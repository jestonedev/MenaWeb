...
namespace MenaWeb.ReportServices
{
    public class ContractReportService : ReportService
    {
        ...
        internal byte[] TakeoverAgreementShare(int idContract, int idSigner, string reportPath, DateTime date, List<Person> persons)
        {
            if (persons.Count == 1)
            {
                var arguments = new Dictionary<string, object>
                ...
                var fileNameReport = GenerateReport(arguments, "mena\\takeover_agreement_share");
                return DownloadFile(fileNameReport);
            }
            else
            {
                ...
                foreach (var person in persons)
                {
                    ...
                    var fileRep = GenerateMultiFileReport(arguments, "mena\\takeover_agreement_share");
                }
                var destFileName = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "files", destDirGuid);
                var destZipFile = destFileName + ".zip";
                if (File.Exists(destZipFile)) File.Delete(destZipFile);
